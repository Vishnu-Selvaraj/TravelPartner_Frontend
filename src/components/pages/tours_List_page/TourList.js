...
function TourList() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (error.response.data.blocked_error) {
        toast.error(error.response.data.blocked_error, {
          style: {
            borderRadius: "10px",
  ...
  const handleSearch = (searchTerm) => {
    if (searchTerm.trim() === "") {
      ...
      var dataRelatedToSearchTerm = tourData.filter((data) =>
        data.place_name.toLowerCase().includes(searchTerm.toLowerCase())
      );
